...
class SyncBase extends Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit = (event) =>{
    this.props.syncMapaComissao(this.state.dateStart, this.state.dateEnd,  this.loading  )
  ...
  render() {
    return (
      <LayoutApp  ref={this.child}  title = "Sincronizar Base"  modal ={{close: true, show:this.state.isShow}} >
        <form className = "sync-base-form" onSubmit={this.handleSubmit}>
          ...
          <div className = "group-button">
                  <button type = 'submit' className="btn">Sincronizar</button>
          </div>
...
export default connect(mapStateToProps, mapDispatchToProps, null, { forwardRef : true } )(SyncBase);
